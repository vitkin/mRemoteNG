...
#Region "Window Overrides and DockPanel Stuff"
    ...
    Protected Overloads Overrides Sub WndProc(ByRef m As Message)
        Try
            Select Case m.Msg
                Case WM_PARENTNOTIFY, WM_SETCURSOR
                    Dim message As Integer
                    Dim position As Point
                    Select Case m.Msg
                        Case WM_PARENTNOTIFY
                            message = m.WParam.ToInt32()
                            position = PointToScreen(New Point(LOWORD(m.LParam), HIWORD(m.LParam)))
                        Case WM_SETCURSOR
                            message = HIWORD(m.LParam)
                            position = MousePosition
                    End Select

                    If Not (message = WM_LBUTTONDOWN Or message = WM_MBUTTONDOWN Or message = WM_RBUTTONDOWN) Then Exit Select

                    Dim control As Control = FromChildHandle(WindowFromPoint(position))
                    Dim interfaceControl As InterfaceControl = TryCast(control, InterfaceControl)
                    If interfaceControl Is Nothing OrElse Not TypeOf interfaceControl.Protocol Is PuttyBase Then Exit Select

                    interfaceControl.Protocol.Focus()

                    Dim connectionWindow As UI.Window.Connection = TryCast(interfaceControl.FindForm, UI.Window.Connection)
                    If connectionWindow IsNot Nothing Then connectionWindow.RefreshIC()
                Case WM_SYSCOMMAND
    ...
    Private Sub pnlDock_ActiveDocumentChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles pnlDock.ActiveDocumentChanged
        ...
        If connectionWindow.TabController Is Nothing OrElse connectionWindow.TabController.SelectedTab Is Nothing Then Return

        Dim interfaceControl As InterfaceControl = TryCast(connectionWindow.TabController.SelectedTab.Tag, InterfaceControl)
        If interfaceControl Is Nothing OrElse interfaceControl.Protocol Is Nothing Then Return

        interfaceControl.Protocol.Focus()
        connectionWindow.RefreshIC()
    End Sub
